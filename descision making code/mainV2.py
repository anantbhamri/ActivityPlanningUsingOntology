...
actionsOfInterest = {
    ...
    # onto.search_one(iri=f"*{action}") for action in [
        ...
        "cs_closeVentActionPlease", "cs_openVentActionPlease",
...
def print_actionsForPath(path, edge_labels,edge_actingAgent, edge_changingStates):
    # Get edge labels for the path
    edge_labels_print = []
    for i in range(len(path) - 1):
        edge_data = G.get_edge_data(path[i], path[i + 1])
        edge_labels_print.append(edge_data.get('relationship', ''))
        actingAgent = edge_actingAgent.get((path[i], path[i + 1]), '')
        statesGoFrom = edge_changingStates.get((path[i], path[i + 1]), '')
        print(f"Action: {edge_labels_print[-1]}, Acting Agent: {actingAgent}, Going from: {statesGoFrom[0]} to {statesGoFrom[1]}")
    # print(f"Edge labels for the path: {edge_labels_print}") 
    return edge_labels_print
...
def findNumberOfSameAgentStateCouplings(coupledWithStates):
    ...
    for state in coupledWithStates:
        agent = get_corresponding_iri(onto, state).correspondingAgent[0]
            ...
            if i != j:
                ...
                if agent_i != agent_j:
                    ...
                    if agentStateDef in composing_st1.is_a[0].ancestors() and agentStateDef in composing_st2.is_a[0].ancestors():
                        ...
                        if len(rel1.constrainingStatesOr) > 0: 
                            hasStatesInCompositeState = composite_st1.hasState
                            constrainingStatesInComposingState = rel1.constrainingStatesOr
...
# For each individual that is belongs to the AgentState Class
# find the constraing states both or and and constraing states
# then for those states find all same state agents and find the composite states

mapAgentStates = {}
mapCouplingBwAgents = {}
mapIndividualStates2ConstrainingORStates = {}
mapIndividualStates2ConstrainingANDStates = {}
...
for individual in onto.individuals():
    ...
    if agentStateDef in indvClass.ancestors():
        ...
        if agentName.name == "A_R":
            print("Debug")
        
        contrainginStatesOr = individual.constrainingStatesOr
        mapIndividualStates2ConstrainingORStates[individual] = contrainginStatesOr
        
        contrainginStatesAnd = individual.constrainingStatesAnd
        mapIndividualStates2ConstrainingANDStates[individual] = contrainginStatesAnd
        setOfconstrainingAgents = set()
        
        for state in contrainginStatesOr:
            agentNameT, agentValueT = find_agent_value(state.name)
            if agentNameT not in setOfconstrainingAgents:
                setOfconstrainingAgents.add(agentNameT)
        
        for state in contrainginStatesAnd:
            agentNameT, agentValueT = find_agent_value(state.name)
            if agentNameT not in setOfconstrainingAgents:
                setOfconstrainingAgents.add(agentNameT)
        
        if len(setOfconstrainingAgents) == 0:
            # add individual to the graph
            G.add_node(individual.name, type="state")
            node_labels[individual.name] = individual.name
          
        if agentName not in mapCouplingBwAgents:
            mapCouplingBwAgents[agentName] = setOfconstrainingAgents
        else:
            mapCouplingBwAgents[agentName].update(setOfconstrainingAgents)

        if agentName not in mapAgentStates:
            mapAgentStates[agentName] = agentName.corrospondedBy

print("Map of Coupling Between Agents: ", mapCouplingBwAgents)
print("Map of Agent States: ", mapAgentStates)

# Function to recursively find all downstream dependencies
def find_all_downstream_dependencies(agent, mapCouplingBwAgents, visited):
    if agent in visited:
        return set()
    visited.add(agent)
    downstream_dependencies = set(mapCouplingBwAgents.get(agent, set()))
    for dependent_agent in mapCouplingBwAgents.get(agent, set()):
        downstream_dependencies.update(find_all_downstream_dependencies(dependent_agent, mapCouplingBwAgents, visited))
    return downstream_dependencies


# Create a map with downstream dependencies
mapCouplingBwAgentsAllDownstream = {}
for agent in mapCouplingBwAgents:
    allCombinations = find_all_downstream_dependencies(agent, mapCouplingBwAgents, set())
    if agent in allCombinations:
        allCombinations.remove(agent)
    mapCouplingBwAgentsAllDownstream[agent] = allCombinations

# Print the map with downstream dependencies
print("Map of Coupling Between Agents with Downstream Dependencies:")
for agent, dependencies in mapCouplingBwAgentsAllDownstream.items():
    print(f"{agent}: {dependencies}")


# Function to generate all possible combinations of states for an agent and its downstream dependencies
def generate_combinations(agent, mapCouplingBwAgentsAllDownstream, mapAgentStates):
    all_states = [mapAgentStates[agent]]
    for dependent_agent in mapCouplingBwAgentsAllDownstream[agent]:
        all_states.append(mapAgentStates[dependent_agent])
    return list(itertools.product(*all_states))

def count_same_elements(tuple1, tuple2):
    return sum(1 for a, b in zip(tuple1, tuple2) if a == b)

def count_different_elements(tuple1, tuple2):
    different_elements = [(i, a, b) for i, (a, b) in enumerate(zip(tuple1, tuple2)) if a != b]
    return len(different_elements), different_elements
...
nominalActions_Agent = {}
nominalActions_Agent["controlPump"] = ["timePassageAction",]
nominalActions_Agent["fuelAmount"] = ["timePassageAction",]
nominalActions_Agent["fuelLevelSensor"] = ["timePassageAction",]
nominalActions_Agent["fuelTruckConnection"] = ["disconnectFuelTruckAction","connectFuelTruckAction",]
nominalActions_Agent["generator"] = ["timePassageAction", "turnOnGeneratorAction",]
nominalActions_Agent["linePower"] = ["powerCutEventReach",]
nominalActions_Agent["reactorCore"] = ["timePassageAction",]
nominalActions_Agent["valve"] = ["closeVentAction", "openVentAction",]
nominalActions_Agent["motionAgent"] = ["goWAction", "goEAction", "goSAction", "goNAction",]
...
addCompositeStates2Ontology = False

# Generate and print all possible combinations of states for each agent
for agent in mapCouplingBwAgentsAllDownstream:

    print("Agent of interest: ", agent)
    if len(mapCouplingBwAgentsAllDownstream[agent]) == 0:
        continue
    # testNewCode
    runNewCombination = True
    useCmbinatorialExplosion = False
    if runNewCombination:
        noOfDownstreamAgents = len(mapCouplingBwAgentsAllDownstream[agent])
        listAll = []
        listAll.append(agent)
        all_states = [mapAgentStates[agent]]
        totalNo = len(mapAgentStates[agent])
        for agent in mapCouplingBwAgentsAllDownstream[agent]:
            listAll.append(agent)
            all_states.append(mapAgentStates[agent])
            totalNo = totalNo * len(mapAgentStates[agent])
        for i in range(0, len(listAll)):
            # create a new list of all states by removing row i
            all_states_new = all_states.copy()
            all_states_new.pop(i)
            allcomb = list(itertools.product(*all_states_new))

            for comb in allcomb:
                for j in range(0,len(all_states[i])):
                    st1 = all_states[i][j]
                    
                    constrainingStatesOR = mapIndividualStates2ConstrainingORStates[st1]
                    constrainingStatesAND = mapIndividualStates2ConstrainingANDStates[st1]


                    if constrainingStatesOR != [] and constrainingStatesAND != []:
                        overallNoPrint = 0
                        # print("Both constraining states are present, check code")
                        continue
                    
                    elif constrainingStatesOR != [] and constrainingStatesAND == []:
                        # check if any of the elements in constrainingStatesOR are in comb1
                        if any(state in comb for state in constrainingStatesOR):
                            overallNoPrint = 0
                            # print("Can perform action from ", comb1, " to ", comb2)
                        else:
                            # print("Cannot perform action from ", comb1, " to ", comb2)
                            continue
                    elif constrainingStatesOR == [] and constrainingStatesAND != []:
                        # check if any of the elements in constrainingStatesAND are in comb1
                        if all(state in comb for state in constrainingStatesAND):
                            overallNoPrint = 0
                            # print("Can perform action from ", comb1, " to ", comb2)
                        else:
                            # print("Cannot perform action from ", comb1, " to ", comb2)
                    ...
                    # find the nominal action between the two states
                    agentClassName = listAll[i].is_a[0].name
                    actionsPossible = nominalActions_Agent[agentClassName]

                    # Now check if two states are connected by a object property as mentioned in actions
                    for action in actionsPossible:
                        actionAdd = "cs_" + action + "Please"
                        allStatesWhichCanBeReached = getattr(st1,action)
                        for st2 in allStatesWhichCanBeReached:
                            if st2 == st1:
                                continue
                            else:
                                # print("Adding edge between ", st1, " and ", st2, " with action ", actionAdd)
                                comb1  = list(comb)
                                comb1.append(st1)
                                comb2 = list(comb)
                                comb2.append(st2)
                            
                                if frozenset(comb1) in mapCompositeStates: 
                                    compositeStateName1 = mapCompositeStates[frozenset(comb1)]
                                    if addCompositeStates2Ontology:
                                        composite_st1 = get_corresponding_iri(onto, compositeStateName1)
                                else:
                                    # Create composite states
                                    compositeStateName1 = f"cs_{noOfCompositeStates}"
                                    noOfCompositeStates += 1
                                    
                                    if addCompositeStates2Ontology:
                                        composite_st1 = onto.compositeState(compositeStateName1)
                                        for state in comb1:
                                            composite_st1.hasState.append(state)
                                    
                                    G.add_node(compositeStateName1, type="compositeState")
                                    node_labels[compositeStateName1] = compositeStateName1
                                    mapCompositeStates[frozenset(comb1)] = compositeStateName1

                                if frozenset(comb2) in mapCompositeStates:
                                    compositeStateName2 = mapCompositeStates[frozenset(comb2)]
                                    if addCompositeStates2Ontology:
                                        composite_st2 = get_corresponding_iri(onto, compositeStateName2)
                                else:  
                                    compositeStateName2 = f"cs_{noOfCompositeStates}"
                                    noOfCompositeStates += 1

                                    if addCompositeStates2Ontology:
                                        composite_st2 = onto.compositeState(compositeStateName2)
                                        for state in comb2:
                                            composite_st2.hasState.append(state)

                                    G.add_node(compositeStateName2, type="compositeState")
                                    node_labels[compositeStateName2] = compositeStateName2
                                    mapCompositeStates[frozenset(comb2)] = compositeStateName2
                                
                                if addCompositeStates2Ontology:
                                    getattr(composite_st1, actionAdd).append(composite_st2)
                                
                                # Add edge to the graph
                                G.add_edge(compositeStateName1, compositeStateName2, relationship=actionAdd)
                                edge_labels[(compositeStateName1, compositeStateName2)] = actionAdd
                                edge_actingAgent[(compositeStateName1, compositeStateName2)] = st1.correspondingAgent[0].name
                                edge_changingStates[(compositeStateName1, compositeStateName2)] = (st1.name,st2.name )

                            if action == "timePassageAction":
                                break

                # Just see the nominal action
                # if allComb has the constraing states then add action
                # print("will do")

    
    
    print("Done 1 set of agent")
    
    if useCmbinatorialExplosion:    
        combinations = generate_combinations(agent, mapCouplingBwAgentsAllDownstream, mapAgentStates)
    
        # For all pairs of combinations:
        for i in range(len(combinations)):
            comb1 = combinations[i]
            for j in range(len(combinations)):
                if i != j:
                    comb2 = combinations[j]
                    # Now check how many elements are different in the two combinations
                    # count_same_elements = count_same_elements(comb1, comb2)
                    different_elements_count, different_elements = count_different_elements(comb1, comb2)
                
                    if different_elements_count == 1:
                        different_st1 = different_elements[0][1]
                        different_st2 = different_elements[0][2]
                        constrainingStatesOR = mapIndividualStates2ConstrainingORStates[different_st1]
                        constrainingStatesAND = mapIndividualStates2ConstrainingANDStates[different_st1]
                        if constrainingStatesOR != [] and constrainingStatesAND != []:
                            overallNoPrint = 0
                            # print("Both constraining states are present, check code")
                            continue
                            ...
                            # check if any of the elements in constrainingStatesOR are in comb1
                            if any(state in comb1 for state in constrainingStatesOR):
                                overallNoPrint = 0
                                # print("Can perform action from ", comb1, " to ", comb2)
                            else:
                                # print("Cannot perform action from ", comb1, " to ", comb2)
                            ...
                            # check if any of the elements in constrainingStatesAND are in comb1
                            if all(state in comb1 for state in constrainingStatesAND):
                                overallNoPrint = 0
                                # print("Can perform action from ", comb1, " to ", comb2)
                            else:
                                # print("Cannot perform action from ", comb1, " to ", comb2)
                        ...
                        for action in actions:
                            actionAdd = "cs_" + action + "Please"
                            if different_st2 in getattr(different_st1,action):
                                # print("Adding edge between ", comb1, " and ", comb2, " with action ", actionAdd)
                                ...
                                if frozenset(comb2) in mapCompositeStates:
                                    ...
                                    node_labels[compositeStateName2] = compositeStateName2
                                    mapCompositeStates[frozenset(comb2)] = compositeStateName2
                                # if compositeStateName1 == "cs_2" and compositeStateName2 == "cs_0":
                                #     print("Debug")
                                getattr(composite_st1, actionAdd).append(composite_st2)
                                # Add edge to the graph
                                G.add_edge(compositeStateName1, compositeStateName2, relationship=actionAdd)
                                edge_labels[(compositeStateName1, compositeStateName2)] = actionAdd
                                edge_actingAgent[(compositeStateName1, compositeStateName2)] = different_st1.correspondingAgent[0].name
                                edge_changingStates[(compositeStateName1, compositeStateName2)] = (different_st1.name,different_st2.name )
...
for nodes in G.nodes():
    # Find all outgoing edges for each node
    outgoing_edges = list(G.out_edges(nodes))
    # Check if any of the edges has label "timePassageActionPlease" or "cs_timePassageActionPlease"
    timePassageActionExists = False
    for edge in outgoing_edges:
        if edge_labels[edge] == "timePassageActionPlease" or edge_labels[edge] == "cs_timePassageActionPlease":
            timePassageActionExists = True
            break
    # If any of the edges has label "timePassageActionPlease" or "cs_timePassageActionPlease", then remove all other edges
    if timePassageActionExists:
        for edge in outgoing_edges:
            if edge_labels[edge] != "timePassageActionPlease" and edge_labels[edge] != "cs_timePassageActionPlease":
                G.remove_edge(edge[0], edge[1])
                actionAdd = edge_labels[edge]
                # also remove the edge in the ontology
                if addCompositeStates2Ontology:
                    composite_st1 = get_corresponding_iri(onto, edge[0])
                    composite_st2 = get_corresponding_iri(onto, edge[1])
                    getattr(composite_st1, actionAdd).remove(composite_st2)
...
# Testing
initialState = ['st_RL', 'st_LPOff', 'st_GOff', 'st_FTNc', 'st_FE', 'st_VC', 'st_CPOff']
initialState_set = set()
for state in initialState:
    initialState_set.add(get_corresponding_iri(onto, state))
initialState_FrozenSet = frozenset(initialState_set)
nodeName_Start = mapCompositeStates[initialState_FrozenSet] 

finalState = ['st_RL', 'st_LPOff', 'st_GOff', 'st_FTNc', 'st_FE', 'st_VC', 'st_CPO']
finalState_set = set()
for state in finalState:
    finalState_set.add(get_corresponding_iri(onto, state))
finalState_FrozenSet = frozenset(finalState_set)
nodeName_End = mapCompositeStates[finalState_FrozenSet] 

paths = find_paths_through_nodes(G, nodeName_Start, nodeName_End, [],[], min_length=1, max_length=10)
print()
print("Initial State: ", initialState )
print_actionsForPath(paths[0], edge_labels,edge_actingAgent, edge_changingStates)
print("Final State: ", finalState )
print("Just printed the shortest path")
...
G.remove_nodes_from(nodes_with_no_outgoing_edges)
...
# mapCouplingBwAgentsAllDownstream = {}
for element in mapCouplingBwAgents:
    print("Agent: ", element)
        
    for state in mapCouplingBwAgents[element]:
        if mapCouplingBwAgents[state] == {}:
            mapCouplingBwAgentsAllDownstream[element] = state
        # else:
...
#         for state in coupledWithStates:
#             stringState = "*" + state
#             agent = onto.search_one(iri=stringState).correspondingAgent[0]
#             if agent.name not in respectiveAgents:
#                 respectiveAgents[agent.name] = [state]
#                 noOfAgents += 1
#             else:
...
#                                 compositeStateIndv.correspondingAgent.append(onto.search_one(iri="*"+key))
#                                 compositeStateIndv.goodState.append(bool_isGoodState)
